...
* 설명
	K Upload는 파일을 전송하는 클라이언트 모듈(Javascript)과 파일을 서버쪽에 저장해 주는 서버 모듈로 구성되어 있습니다.
...
* K Upload 폴더 설명
    - agent
	    K Upload 설치파일을 포함하고 있습니다.
	- config	
		K Upload의 환경설정을 담당하는 raonkupload.config.xml 을 포함하고 있습니다.
	- css
		K Upload에 필요한 스타일시트 파일을 포함하고 있습니다.
	- raonkuploaddata
		저장경로에 대한 별도 설정이 없다면, 파일이 저장되는 ROOT 경로입니다.
	- handler
		파일을 서버에 저장하는 서버 모듈을 포함하고 있습니다. (.NET, JAVA 모듈)
	- images
		K Upload에 필요한 이미지 파일을 포함하고 있습니다.
	- js	
		K Upload에 필요한 자바스크립트 파일을 포함하고 있습니다.
	- sdk
		K Upload의 SDK를 포함하고 있습니다. (개발 및 테스트 완료후 삭제 하셔도 됩니다.)

* 설치방법
	아래의 순서에 따라 설치하십시오.

	1. config\raonkupload.config.xml 설정
		RaonKUpload > license > product_key, license_key 에 발급받은 키를 설정합니다.

	2. K Upload Core 전체 폴더 업로드(하위폴더 및 파일포함)
		WEB서버와 WAS가 분리되어 있는 경우 handler, handler\JAVA, handler\.NET 폴더는 WAS서버에 업로드 합니다.
		나머지 폴더는 WEB서버에 설치합니다.(WEB서버와 경로를 맞춰 WAS에 설치합니다.)

	3. 웹 언어별 설정
		1) ASP, ASP.NET : 
			config\raonkupload.config.xml 설정 : RaonKUpload > uploader_setting > develop_langage 에 NET 으로 설정합니다.
		...
		2) JSP : 
			config\raonkhandler.config.xml 설정 : RaonKUpload > uploader_setting > develop_langage 에 JAVA 로 설정합니다.
	...
	4. 권한설정
		...
		1) 환경 설정 정보가 저장되는 파일
			ex) config\raonkupload.config.xml
...
* K Upload 서버 모듈 구성
	기본적으로 파일을 업로드하면, 기본 설치폴더/raonkuploaddata 폴더에 파일이 업로드가 되며, WEB 서버와 WAS가 분리된 경우 WAS쪽 폴더(/raonkuploaddata)에 업로드 됩니다.
...
* K Upload를 생성하는 방법
	1) <head>...</head> 태그 사이에 아래의 코드를 추가합니다.
		...
		<script type="text/javascript" src="RAONKUpload설치폴더URL/js/raonkupload.js">

	2) <body>...</body> 태그 사이에 업로드가 들어갈 위치에 업로드 생성 코드를 삽입합니다.

		<script type="text/javascript">
			var upload = new RAONKUpload({Id: "생성할 업로드 이름"});
		</script>
