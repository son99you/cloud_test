<%@page import="com.raonwiz.kupload.RAONKHandler"%><%@page import="com.raonwiz.kupload.event.*"%><%@page import="com.raonwiz.kupload.util.EventVo"%><%@page import="com.raonwiz.kupload.common.ImageApi"%><%@ page contentType="text/html;charset=utf-8"%><%
	...
	// 업로드 시 클라이언트에서 AddFormaData로 추가한 값을 받기 위하여 활성화
	event.addBeforeUploadEventListener(new BeforeUploadEventListener() {
		public void beforeUploadEvent(EventVo eventVo) {
		    ...
		    // name1 의 이름을 가진 formdata를 받음.
		    String[] formData1 = eventVo.getUpload().getParameterValue("name1");
		    if (formData1 != null && formData1.length > 0) {
		        upload_formDataValue1 = formData1[0];
		    }
		    
		    // name2 의 이름을 가진 formdata를 받음.
		    String[] formData2 = eventVo.getUpload().getParameterValue("name2");
		    if (formData2 != null && formData2.length > 0) {
		        upload_formDataValue2 = formData2[0];
		    }
		    /*************************************************************************************/
	...
	// 다운로드 시 클라이언트에서 추가한 formdatavalue 값을 받기 위하여 활성화
