...
function addHandle(o, win) {
    ...
    if (typeof(win.DIF_mainHandlersAdded)=="undefined" || !win.DIF_mainHandlersAdded) {
        ...
        with (win) {
            eval("function OnMouseDownHandler(evt) { if(typeof(evt)=='undefined'){evt=event;}"+topRefStr+".DIF_begindrag(evt, "+topRefStr+") }");
            eval("document.onmousedown = OnMouseDownHandler;");
            eval("function OnMouseUpHandler(evt) { if(typeof(evt)=='undefined'){evt=event;}"+topRefStr+".DIF_enddrag(evt, "+topRefStr+") }");
            eval("document.onmouseup = OnMouseUpHandler;");
            eval("function OnMouseMoveHandler(evt) { if(typeof(evt)=='undefined'){evt=event;}"+topRefStr+".DIF_iframemove(evt, "+topRefStr+") }");
...
function DIF_getIframeId(win) {
    /*
     ...
     return null;
     */
    DIF_iframeObjects['yettie_iframe'] = window.parent.document.getElementById('yettie_iframe');
    return 'yettie_iframe';
}
...
function DIF_drag(x,y) {
    ...
    var _frame = parent.document.getElementById('yettie_iframe');
    var _top = parseInt(_frame.height.substring(0, _frame.height.length)) / 2;
    var _left = parseInt(_frame.width.substring(0, _frame.width.length)) / 2;
    var _iframeWidth = parseInt(window.parent.document.getElementById('yettie_iframe').width);
    var _iframeHeight = parseInt(window.parent.document.getElementById('yettie_iframe').height);
    var innerWidth = parent.window.innerWidth ? parent.window.innerWidth : parent.document.documentElement.clientWidth ? parent.document.documentElement.clientWidth : screen.width;
    var innerHeight = parent.window.innerHeight ? parent.window.innerHeight : parent.document.documentElement.clientHeight ? parent.document.documentElement.clientHeight : screen.height;
    ...
    //초기.
    if((newPositionX == 0 || newPositionX == 1) && (newPositionY == 0 || newPositionY == 1)){
        newPositionX = innerWidth / 2;
        newPositionY = innerHeight / 2;
    ...
    //벗어나지않게..
    if(newPositionX - _left < 0){
        DIF_iframeObjects[DIF_iframeBeingDragged].style.left = _left + "px";
    }
    else if(newPositionX - _left > innerWidth - _iframeWidth){
        DIF_iframeObjects[DIF_iframeBeingDragged].style.left = (innerWidth - _iframeWidth + _left) + "px";
    }
    else{
        DIF_iframeObjects[DIF_iframeBeingDragged].style.left = newPositionX + "px";
    }

    if(newPositionY - _top < 0){
        DIF_iframeObjects[DIF_iframeBeingDragged].style.top = _top + "px";
    }
    else if(newPositionY - _top > innerHeight - _iframeHeight){
        DIF_iframeObjects[DIF_iframeBeingDragged].style.top = (innerHeight - _iframeHeight + _top) + "px";
    }
    else{
        DIF_iframeObjects[DIF_iframeBeingDragged].style.top  = newPositionY + "px";
    }

    //console.log(DIF_iframeObjects[DIF_iframeBeingDragged].style.left);
    //console.log(DIF_iframeObjects[DIF_iframeBeingDragged].style.top);
