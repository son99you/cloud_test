...
(function() {
	...
	pageObj.deptList = function() {
		FwkCmmnUtil.submitForm(defaultFrm, "/comm/popup/deptYMList.do");
	};
	 ...
	 *  =======================================================   
	 */
	setdeptInfo = function(YM, STORE, ORGAN, DEPT_NM) {
		window.opener.deptListAdd(YM, STORE, ORGAN, DEPT_NM);
	   self.close();
	};
	...
	P_YM_SSetting = function() {
		var date = new Date();
		var yyyy = date.getFullYear()-1;
		var MM = date.getMonth()+1;
		
		var dateS = new Date(yyyy, MM, 0); 
		...
		for(var i = 1; i <= 12; i++) {
			...
			if($('#D_YM').val() == yyyy+mmStr) {
				var option = $("<option value=\""+ yyyy+mmStr +"\" selected>" + yyyy+mmStr + "</option>");
				$('#P_YM_S').append(option);
			}else {
				var option = $("<option value=\""+ yyyy+mmStr +"\">" + yyyy+mmStr + "</option>");
				$('#P_YM_S').append(option);
