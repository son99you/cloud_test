...
(function() {
	...
	pageObj.bsnsDeptListInqire = function() {
		FwkCmmnUtil.submitForm(defaultFrm, "/comm/popup/unitInqireList.do");
	};
	 ...
	 *  =======================================================   
	 */
	//setUnitInfo('${data.CD_VALUE}', '${data.CD_VALUE_NM}');"</c:if>
	setUnitInfo = function(cdValue, cdValueNm) {
		var btnId = $("#btnId").val();
		
		if(btnId ==""){
			if($("#cdValue",opener.document)){
				$("#cdValue",opener.document).val(cdValue);
			};
			if($("#cdValueNm",opener.document)){
				$("#cdValueNm",opener.document).val(cdValueNm);
			};
		}else{
			if($("#cdValue"+btnId,opener.document)){
				$("#cdValue"+btnId,opener.document).val(cdValue);
			};
			if($("#cdValueNm"+btnId,opener.document)){
				$("#cdValueNm"+btnId,opener.document).val(cdValueNm);
			};
		...
		try {
			window.opener.gridUnitAdd(cdValue, cdValueNm);	
		}catch(e) {
	...
	pageObj.setEventHandler = function() {
		...
		FwkCmmnUtil.setEnterKeyBinding(defaultFrm, ["P_CD_VALUE_NM_S","select2"], function() {
