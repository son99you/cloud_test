...
(function() {
	...
	pageObj.cmtmMngProgRegist = function() {
		fileUploadStart();
	};
	...
	fileUploadStart = function() {
		...
		if (RAONKUPLOAD.GetTotalFileCount("upload") == 0) {
			FwkCmmnUtil.submitForm("registFrm", "/estm/cmtmMngProgRegist.do");
        } else {
...
//			$("#prioRnkSlctPeSrchBtn").css("display", "none");   // 우선순위선정자 검색 버튼
	...
	pageObj.setEventHandler = function() {
		...
		$("#saveBtn").on("click", function() {
			...
			if($("input[name='P_ESTM_INFO_CNTC_NO']").val() == ""){
				alert("[평가정보연계번호] 항목은 필수입력 입니다.");
				$("input[name='P_ESTM_INFO_CNTC_NO']").focus();
				return false;
			}
	...
	$(function() {
		...
		fileUpload("registFrm", "P_FILE_GRP_NO", "/estm/cmtmMngProgRegist.do", "detailFrm", "/estm/cmtmMngProgDetail.do", "");
	});
})();