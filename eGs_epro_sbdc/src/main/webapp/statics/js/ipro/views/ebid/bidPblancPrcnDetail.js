...
/*	pageObj.bidPblancPrcnUpdtForm = function() {
	...
	pageObj.bidReqProgStat = function(obj, rmk) { 
		...
		$("#updtFrm input[name='P_BID_PSCD_TEXT']").val(rmk);	//상세상태 내용
	...
	// 입찰취소 팝업
	pageObj.bidReqCanclePopup = function() {
		$("#popupFrm").one("submit", function() {
			window.open("", "bidReqCanclePopup", "width=750px,height=500px,toolbar=no,status=no,scrollbars=yes,resizeable=no,menubar=no,left=500,top=100");
			this.action = FwkCmmnUtil['contextPath']+'/ebid/popup/bidReqCanclePopup.do';
	        this.method = 'POST'; 
	        this.target = 'bidReqCanclePopup';
	    }).trigger("submit"); 
	}; 
	reList = function() {
		FwkCmmnUtil.submitForm("updtFrm", "/ebid/bidPblancPrcnList.do");
	} 
	...
	pageObj.setEventHandler = function() {
		...
		$("#saveBtn").on("click", function() {
			...
			/*// 필수체크
	        if($("#P_BFAN_NM").val() == ""){
	        	alert("[공고명] 항목은 필수입력입니다.");
	        	$("#P_BFAN_NM").focus();
	        	return false;
	        }
	        
	        if($("select[name='P_CONT_SECD']").val() == ""){
	        	alert("[계약구분] 항목은 필수입력입니다.");
	        	$("select[name=P_CONT_SECD]").focus();
	        	return false;
	        } 
	        
	        if($("select[name='P_CONT_MTCD']").val() == ""){
	        	alert("[계약방법] 항목은 필수입력입니다.");
	        	$("select[name=P_CONT_MTCD]").focus();
	        	return false;
	        }
	        
	        if($("select[name='P_SBID_MTCD']").val() == ""){
	        	alert("[낙찰방법] 항목은 필수입력입니다.");
	        	$("select[name=P_SBID_MTCD]").focus();
	        	return false;
	        }
	        
	        if($("#P_BFAN_STDE").val() == ""){
	        	alert("[사전규격공개기간 시작일] 항목은 필수입력입니다.");
	        	$("#P_BFAN_STDE").focus();
	        	return false;
	        }
	        ...
	        if($("#P_BFAN_ENDE").val() == ""){
	        	alert("[사전규격공개기간 종료일] 항목은 필수입력입니다.");
	        	$("#P_BFAN_ENDE").focus();
	        	return false;
	        }*/
			...
			pageObj.bidReqPrcnUpdtForm();
		}); 
		// 공고버튼
		$("#annoBtn").on("click", function() {
			
			if($("input[name='P_REAL_ANNC_NO']").val() == ""){
				alert("입찰공고번호를 입력해주세요."); 
				$("input[name='P_REAL_ANNC_NO']").focus();
				return false;
			}
			
			if(!confirm("입찰을 공고 하시겠습니까?")){
				return false;
			}
			pageObj.bidReqProgStat("A008","");
		});
...
//			if(!confirm(FwkMssageUtil.getMessage("COM.CON.001", "입찰취소"))){
//				return false;
//			}
//			pageObj.bidReqProgStat("A023","입찰취소");
			$("#popupFrm input[name='P_BID_PSCD']").val("A023");
			pageObj.bidReqCanclePopup();
		});
		...
		/*	if($("input[name='P_REAL_ANNC_NO']").val() == ""){
			...
			if (!confirm("입찰공고요청을 접수 하시겠습니까?")) {
				return false;
			} 
			pageObj.bidReqProgStat("A004",""); 
		}); 

		// 요청반려
		$("#rqstRtnBtn").on("click", function() {
			$("#popupFrm input[name='P_BID_PSCD']").val("A005");
		...
		$("#RecApplyBtn").on("click", function() {
			if (!confirm("입찰공고요청접수를 승인 하시겠습니까?")) {
				return false;
			}
			pageObj.bidReqProgStat("A006","");
		});

		// 접수반려
		$("#RecRtnBtn").on("click", function() {
			$("#popupFrm input[name='P_BID_PSCD']").val("A007");
			pageObj.bidReqRtnPopup();
		});
		
		// 단순반려
		$("#rtnBtn").on("click", function() {
			$("#popupFrm input[name='P_BID_PSCD']").val("A028");
			pageObj.bidReqRtnPopup();
		});
...
//		}; 
		if($("#contMtcd").val() != "10004"){	//일반이나 수의 일경우에만 보임
			$(".essfileShowList").show(); 
		}
