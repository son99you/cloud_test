...
(function() {
	...
	pageObj.entrpsList = function() {
		...
		if($("#P_VEND_NM_S").val() != "" || $("#P_BIZRNO_S").val() != ""|| $("#P_VEND_REG_NO_S").val() != ""){
			FwkCmmnUtil.submitForm(defaultFrm, "/comm/popup/entrpsList.do");
		}else{
			alert("조회조건을 입력해주세요.");
		}
	};	
	...
	setEntrpsInfo = function(vendRegNo,bizrno, vendNm, rprsNm, addrNm, telNo, emalAddr) {	
		try {
			window.opener.entrpsListAdd(vendRegNo, bizrno, vendNm, rprsNm, addrNm, telNo, emalAddr, "N");
		}catch(exception) {
	...
	pageObj.ccpyListRegist = function() {
		...
		window.opener.entrpsListAdd(arr_vend_reg_no, arr_bizrno, arr_vend_nm, arr_rprs_nm,arr_addr_nm,arr_tel_no,arr_emal_addr,"Y");
		self.close();
	};
