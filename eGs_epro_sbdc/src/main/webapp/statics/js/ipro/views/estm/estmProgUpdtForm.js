...
//        }
	...
	pageObj.estmSecdSetting = function(){
		...
		var D_ESTM_SECD = $("#D_ESTM_SECD").val();
		...
		FwkCmmnUtil.submitAjax (actionUrl, jsonData
			...
			if(resultCode == 1){   // 성공
				...
				$.each(list, function(inx, item){
					...
					if(D_ESTM_SECD == item.CD_DTL_ID) {
						option += "<option value='"+item.CD_DTL_ID+"' selected >"+item.CD_DTL_NM+"</option>";
					}else {
...
//			$("#prioRnkSlctPeSrchBtn").css("display", "none");   // 우선순위선정자 검색 버튼
	...
	$(function() {
		...
		pageObj.outCmtmSlctMthdEvent();
		pageObj.innCmtmSlctMthdEvent();
		fileModify("updtFrm", "P_FILE_GRP_NO", "P_FILE_GRP_NO_NEW", "/estm/estmProgUpdt.do");//, "detailFrm", "/estm/estmProgDetail.do", "");
	});
})();