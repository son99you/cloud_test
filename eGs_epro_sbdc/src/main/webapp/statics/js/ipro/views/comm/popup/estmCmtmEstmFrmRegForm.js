...
//	ESTM_DTL_ITEM_ARR[0][0] = "01";
	...
	pageObj.setEventHandler = function() {
		...
		$("#estmEndBtn").on("click", function() {
			FwkCmmnUtil.submitForm("listFrm" , "/comm/popup/estmCmtmEstmFrmRegForm.do");
			return false;
		});
		...
		$(document).on("click","#radioId", function() {
			...
			var nameKey = $(this).parent().parent().find($("input[name='NAME_KEY']")).val();
			
			/**
			 * 1. 같은 이름의 P_ESTM_DTL_ITEM_POINT+nameKey 값의 히든값을 지우고
			 * 2. 체크된 값에는 현재 데이터 집어넣기 
			 */
			//1.
			$("input:radio[name='P_ESTM_DTL_ITEM_POINT" + nameKey + "']").each(function(i) {
				$(this).parent().parent().find($("input[name='P_ESTM_DTL_ITEM_POINT']")).val("");
			});

			//2.			
			$(this).parent().parent().find($("input[name='P_ESTM_DTL_ITEM_POINT']")).val($(this).val());
	...
	$(function() {		
		...
		if($("#resultCode").val() == 'Success'){
			FwkCmmnUtil.submitForm("registFrm", "/comm/popup/estmCmtmEstmFrmRegForm.do");
		}
		
		//fileUpload("registFrm", "P_FILE_GRP_NO", "/comm/popup/estmCmtmEstmFrmSave.do");
		fileModify("registFrm", "P_FILE_GRP_NO", "P_FILE_GRP_NO_NEW", "/comm/popup/estmCmtmEstmFrmSave.do");
