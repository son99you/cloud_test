...
(function() {
	...
	pageObj.bfStndRqstPrcnList = function() {
		...
		FwkCmmnUtil.submitForm(defaultFrm, "/ebid/bfStndOpenComplList.do");
	};
	...
	detailInqire =  function(bfanNo) {
			...
			FwkCmmnUtil.submitForm("detailFrm", "/ebid/bfStndOpenComplDetail.do");
	};
	...
	pageObj.bfanComplExcelDwld = function(){
		$("select[name='P_ARA_DEPT_CD_S']").prop("disabled",false);
		FwkCmmnUtil.submitForm("searchFrm", "/ebid/bfanComplExcelDwld.do");
	...
	pageObj.setEventHandler = function() {
		...
		$("#excelBtn").on("click", function() {
			if(!confirm("조회된 사전규격완료건에 대하여 엑셀 다운로드 하시겠습니까?")) {
				return false;
			}
			pageObj.bfanComplExcelDwld();
		});
