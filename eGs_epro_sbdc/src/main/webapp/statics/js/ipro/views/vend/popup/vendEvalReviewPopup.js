...
//	ESTM_DTL_ITEM_ARR[6][0] = "0303";
	...
	addItemHtml = function(btnGbn, itemArrLength) {
		...
		for(var n = 0; n < sortX; n++) {
			...
			strHtml  += "<td class=\"txtc\"><textarea style=\"width: 100%; height: 70px;\" name=\"P_ESTM_ITEM_DTL_CNTN\">"+ ESTM_DTL_ITEM_ARR[itemLine][2] +"</textarea></td>";
			strHtml  += "<td class=\"txtc\">" + ESTM_DTL_ITEM_ARR[itemLine][3] +"</td>";
			strHtml  += "<td class=\"txtc\">";
			
			console.log("ESTM_DTL_ITEM_ARR[i][4] :: " + ESTM_DTL_ITEM_ARR[itemLine][4]);
			if(ESTM_DTL_ITEM_ARR[itemLine][4] == "A") // 합산
			{
				strHtml  += "<input type=\"text\" style=\"width : 10%;\" numeric name=\"P_ESTM_DTL_ITEM_POINT\" >";
			}else if (ESTM_DTL_ITEM_ARR[itemLine][4] == "B") // 최고점
			{
				//strHtml  += "<input type=\"radio\" class=\"vam mr5\" name=\"P_ESTM_DTL_ITEM_POINT\" value=\"" + ESTM_DTL_ITEM_ARR[itemLine][3] + "\">";
				var nameKey = ESTM_DTL_ITEM_ARR[itemLine][0].substring(0,myItemLen-2);
				console.log("nameKey :: "+ nameKey);
				
				strHtml  += "<div class=\"rad_g\">";
					strHtml  += "<div class=\"radio_box\">";
						strHtml  += "<input type=\"radio\" name=\"P_ESTM_DTL_ITEM_POINT" +nameKey+ "\" id=\"radioId" + ESTM_DTL_ITEM_ARR[itemLine][0] + "\" value=\"" + ESTM_DTL_ITEM_ARR[itemLine][3] + "\" checked=\"\">";
	...
	$(function() {		
		...
		$("input[name='P_ESTM_DTL_ITEM_NO']", opener.document).each(function(i) {
			ESTM_DTL_ITEM_ARR[i] = new Array(5);
			ESTM_DTL_ITEM_ARR[i][0] = $(this).val();
			console.log("0 ::" + ESTM_DTL_ITEM_ARR[i][0]);
		});
		
		$("input[name='P_ESTM_DTL_ITEM_NM']", opener.document).each(function(i) {
			ESTM_DTL_ITEM_ARR[i][1] = $(this).val();
			console.log("1 ::" + ESTM_DTL_ITEM_ARR[i][1]);
		});
		
		$("textarea[name='P_ESTM_ITEM_DTL_CNTN']", opener.document).each(function(i) {
			ESTM_DTL_ITEM_ARR[i][2] = $(this).val();
			console.log("2 ::" + ESTM_DTL_ITEM_ARR[i][2]);
		});
		
		$("input[name='P_ESTM_DTL_ITEM_SCR']", opener.document).each(function(i) {
			ESTM_DTL_ITEM_ARR[i][3] = $(this).val();
			console.log("3 ::" + ESTM_DTL_ITEM_ARR[i][3]);
		});
		
		$("select[name='P_ESTM_DTL_ITEM_SCR_SECD']", opener.document).each(function(i) {
