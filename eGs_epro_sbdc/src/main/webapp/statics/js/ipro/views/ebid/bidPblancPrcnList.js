...
(function() {
	...
	pageObj.bidPblancPrcnList = function() {
		FwkCmmnUtil.submitForm(defaultFrm, "/ebid/bidPblancPrcnList.do");
	};
	...
	pageObj.bidPblancPrcnExcelDwld = function(){
		$("select[name='P_ARA_DEPT_CD_S']").prop("disabled",false);
		FwkCmmnUtil.submitForm("searchFrm", "/ebid/bidPblancPrcnExcelDwld.do");
	}
	/**
	...
	pageObj.setEventHandler = function() {
		...
		$("#excelBtn").on("click", function() {
			if(!confirm("조회된 입찰공고진행현황건에 대하여 엑셀 다운로드 하시겠습니까?")) {
				return false;
			}
			
			pageObj.bidPblancPrcnExcelDwld();
