(function() {
	...
	pageObj.paySubmit = function(obj) {
		...
		if($(obj).parent().parent().find($("input[name='P_ASSESS_AMT']")).val() == "") {
			alert("지급요청시 지급금액은 필수 입니다.");
			return false;
		}
		...
		$("#PayFrm input[name='P_ASSESS_AMT']").val($(obj).parent().parent().find($("input[name='P_ASSESS_AMT']")).val()); // 지금금액
	...
	deptListAdd = function(YM,STORE, ORGAN, DEPT_NM){
		...
		$("#P_PAY_ORGANSUM" + selectNumber).val(STORE + ORGAN); // 집행부서코드(organ)
	...
	pageObj.setEventHandler = function() {
		 ...
		 * 개인정보삭제버튼 클릭 이벤트
		 */
		$("#rsdnNoDelBtn").on("click", function() {
			
			if(!confirm("평가위원 개인정보를 삭제하면 더 이상 수당지급을 할 수 없습니다.\n" +
						"정말 평가위원 개인정보를 삭제하시겠습니까?")){
				return false; 
			}
			
			FwkCmmnUtil.submitForm("detailFrm" , "/estm/estmRsdnNoDelete.do");
			return false;
		});
