...
(function() {
	...
	pageObj.budgetList = function() {
		FwkCmmnUtil.submitForm(defaultFrm, "/opro/comm/popup/budgetList.do");
	};	
	...
	setbudgetInfo = function(deptNm, deptNo, bsnsCd, bsnsNm, acntCd, acntNm, acntItemCd,rqstBlncAmt, bdgAmt, expsRqstAmt, useBdgAmt,bdgYr, bdgMm) {
		//선택담당자수, 부서명, 부서코드, 직책명, 직책코드, 담당자명, 담당자ID, 전화번호, 이메일
		try {
			window.opener.budgetListAdd(deptNm, deptNo, bsnsCd, bsnsNm, acntCd, acntNm, acntItemCd,rqstBlncAmt, bdgAmt, expsRqstAmt, useBdgAmt,bdgYr, bdgMm);	
		}catch(exception) {
	...
	pageObj.ccpyListRegist = function() {
		...
		window.opener.budgetListAdd(ccpyCnt, arr_dept_nm, arr_dept_no, arr_bsns_cd, arr_bsns_nm,arr_acnt_cd,arr_acnt_nm,arr_acnt_item_cd,arr_rqst_blnc_amt,arr_bdg_amt, arr_exps_rqst_amt, arr_use_bdg_amt, arr_bdg_yr, arr_bdg_mm);
