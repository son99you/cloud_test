...
(function() {
	...
	pageObj.chargerListInqire = function() {
		FwkCmmnUtil.submitForm(defaultFrm, "/opro/comm/popup/chargerList.do");
	};
	...
	setchargerInfo = function(usrId, emplNo,usrNm, deptCd, deptNm, ofpsCd, ofpsNm, telNo, emalAddr) {
		try {
			window.opener.chargerListAdd(usrId, emplNo,usrNm, deptCd, deptNm, ofpsCd, ofpsNm, telNo, emalAddr);	
		}catch(exception) {
	...
	pageObj.chargerListRegist = function(){
		...
		try {
				window.opener.chargerListAdd(chargerCnt, arr_usr_id,  arr_empl_no, arr_usr_nm, arr_dept_cd, arr_dept_nm, arr_ofps_cd, arr_ofps_nm, arr_tel_no, arr_emal_addr);
		}catch(exception) {
			//console.log("부모객체에 function 없으면 통과(별다른 문제 없음)");
		}
		self.close();
