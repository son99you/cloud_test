(function() {
	...
	pageObj.joinRegist = function () {
		FwkCmmnUtil.submitForm("registFrm", "/opro/main/joinRegist.do");  
	};
	...
	pageObj.itemListPopup = function() {
		$("#itemListPopupFrm").one("submit", function() {
			...
			this.action = FwkCmmnUtil['contextPath']+'/opro/comm/popup/itemList.do';
	        this.method = 'POST';
	...
	chargerList = function() {
		$("#popupFrm").one("submit", function() {
			window.open("", "chargerList", "width=750px,height=760px,toolbar=no,status=no,scrollbars=yes,resizeable=no,menubar=no,left="+x(740)+",top=100");
			this.action = FwkCmmnUtil['contextPath']+'/opro/comm/popup/chargerList.do';
	        this.method = 'POST';
