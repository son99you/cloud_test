...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bidPblancResultDetail.js"></script>
...
<div class="sp_sec">
	...
	<form id="updtFrm" method="POST" enctype="multipart/form-data">
		...
		<input type="hidden" id="P_BID_PSCD_TEXT" name="P_BID_PSCD_TEXT">
		 ...
		 <!-- 수의계약여부 -->
		<input type="hidden" id="P_PRVT_CONT_YN" name="P_PRVT_CONT_YN">
		<input type="hidden" id="P_ARAM_GBN" name="P_ARAM_GBN">
		...
		<!-- 입찰진행순서 end-->
				...
				<br>
				<div class="essfileShowList tit_area" style="margin-top: 10px;">
			...
			<div class="btm_btns">
				<c:if test="${bidReqDetail.BID_PSCD eq 'A022' && empty bidReqDetail.PRVT_CONT_YN}"><!-- 유찰상태일 경우  -->
					<button type="button" class="btn ty02" id="reBidBtn">재공고</button>
					<button type="button" class="btn ty02" id="contProgBtn">수의계약</button>
				</c:if>
				<c:if test="${bidReqDetail.BID_PSCD eq 'A022' && not empty bidReqDetail.PRVT_CONT_YN && bidExistCnt eq 0}"><!-- 유찰상태일 경우  -->
					<c:if test="${bidReqDetail.PRVT_CONT_YN eq 'N' }">
						<button type="button" class="btn ty02" id="reBidCnlBtn">재공고취소</button>
					</c:if>
					<c:if test="${bidReqDetail.PRVT_CONT_YN eq 'Y' }">
						<button type="button" class="btn ty02" id="contProgCnlBtn">수의계약취소</button>
					</c:if>
				</c:if>
