...
<div class="pop_wrap">
	...
	<div class="pop_container">
		<div class="view_wrap typeC">
			...
			<div class="view_area">
		    	...
		    	<colgroup>
			        ...
			        <tbody>
				    	<tr class="line">
			                <th>공고번호</th>
			                <td>${entrpsBidBassInfoInqire.ANNC_NO}-${entrpsBidBassInfoInqire.ANNC_NGR}</td>
			                <th>공고일자</th>
			                <td>${comFn:formatDate(entrpsBidBassInfoInqire.ANNC_DT,'yyyyMMddHHmmss','yyyy-MM-dd')}</td>
			            </tr>
			            <tr>
			                <th>입찰명</th>
			                <td colspan="3">${entrpsBidBassInfoInqire.BID_NM}</td>
			            </tr>
			            <tr>
			                <th>계약구분</th>
			                <td>
			                	${entrpsBidBassInfoInqire.CONT_SECD_NM}<c:if test="${entrpsBidBassInfoInqire.UPRC_YN eq 'Y'}">(단가)</c:if>
				            	<c:if test="${bidPartcptReqstdocInqire.CONT_DECD eq '200'}">&nbsp;/&nbsp;${entrpsBidBassInfoInqire.CONT_DECD_NM}</c:if>
			                </td>
			                <th>계약방법</th>
			                <td>${entrpsBidBassInfoInqire.CONT_MTCD_NM} </td>
			            </tr>
			            <tr>
			                <th>대표업체명</th>
			                <td colspan="3">${entrpsBidBassInfoInqire.VEND_NM}</td>
			            </tr>
		            </tbody>
	    		</table>
			</div>
			...
			<div class="tit_area">
				<%-- <h4 class="tit">공동수급 방식 : ${entrpsBidBassInfoInqire.ASSO_SPDM_CD_NM}</h4> --%>
