...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/cmtmRnkSlctProgList.js"></script> 
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">평가위원순위선정진행현황 목록</li>
				</ul>
			</div>
...
<div class="page-list">
	<form id="searchFrm" name="searchFrm" method="post">
		...
		<div class="form-setting-box">
			...
			<div class="ui-setting">
				...
				<%-- <div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_PSCD_S" name="P_ESTM_PSCD_S" selectKey="${param.P_ESTM_PSCD_S}" cdId="ESTM_PSCD"  headerKey="" cond2="RNK_SLCT_PROG" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div> --%>
				<!-- Form Setting -->
			...
			<div class="table-option">
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(cmtmRnkSlctProgListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/estm/cmtmRnkSlctProgListExcelDwld.do','${comFn:nvl(cmtmRnkSlctProgListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<div style="overflow-x: scroll; overflow-y:hidden" >
				<table class="component-table">
					...
					<tbody>
						...
						<!-- //엑셀다운로드데이터 종료 -->
						<c:if test="${comFn:nvl(cmtmRnkSlctProgListTotCnt, 0) == 0}">
							<tr>
								<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						<c:if test="${cmtmRnkSlctProgListTotCnt > 0}">
							<c:forEach var="data" items="${cmtmRnkSlctProgList}" varStatus="status">
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(cmtmRnkSlctProgListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
