...
<script type="text/javascript" src="${jsPath}/ipro/views/comm/popup/estmCmtmEstmFrmBDetail.js"></script>
...
<!-- //서식데이터세팅 -->
	...
	<form id="registFrm" class="search_form" method="POST">
		...
		<input type="hidden" id="P_ESTM_NO" name="P_ESTM_NO" value="${estmCmtmEstmFrmDetail.ESTM_NO}">
		<input type="hidden" id="P_ESTM_PROCD_SEQ" name="P_ESTM_PROCD_SEQ" value="${estmCmtmEstmFrmDetail.ESTM_PROCD_SEQ}">
		<input type="hidden" id="P_ESTM_OBJ_SEQ" name="P_ESTM_OBJ_SEQ" value="${estmCmtmEstmFrmDetail.ESTM_OBJ_SEQ}">
		<input type="hidden" id="P_ESTM_CMTM_NO" name="P_ESTM_CMTM_NO" value="${estmCmtmEstmFrmDetail.ESTM_CMTM_NO}">
		...
		<div class="bottom-buttons" style="">
			<c:if test="${not empty estmObjDetail.PREV_ESTM_OBJ_SEQ }">
				<button type="button" name="prevBtn"  style="float: left; margin-bottom: 10px;" class="btn-bottom type-a">이전</button>
			</c:if>
