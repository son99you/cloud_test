...
<%--
 * 평가관리 > 평가위원관리 > 평가위원평가완료현황 목록
 *
 * <pre>
 * estm
 *    |_ cmtmMngCmplList.jsp
 * 
 * </pre>
 * @date : 2021. 03. 23.
 * @version : 1.0
...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/cmtmMngCmplList.js"></script> 
...
<div class="page-list">
	<form id="searchFrm" name="searchFrm" method="post">
			...
			<div class="table-option">
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(cmtmCmplListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/estm/cmtmMngCmplListExcelDwld.do','${comFn:nvl(cmtmCmplListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<div style="overflow-x: scroll; overflow-y:hidden" >
				<table class="component-table">
					...
					<tbody>
						...
						<!-- //엑셀다운로드데이터 종료 -->
						<c:if test="${comFn:nvl(cmtmCmplListTotCnt, 0) == 0}">
							<tr>
								<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						
						<c:if test="${cmtmCmplListTotCnt > 0}">
							<c:forEach var="data" items="${cmtmCmplList}" varStatus="status">
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(cmtmCmplListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
...
<form id="detailFrm" method="POST">
	...
	<input type="hidden" name="P_ESTM_NO" value="">
</form>
