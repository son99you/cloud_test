...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/estmProgList.js"></script>
	...
	<form id="searchFrm" name="searchFrm" method="post">
		...
		<div class="form-setting-box">
			...
			<div class="ui-setting">
				...
				<div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_SECD_S" name="P_ESTM_SECD_S" selectKey="${param.P_ESTM_SECD_S}" cdId="ESTM_SECD"  headerKey="" cond1="ESTM_PROG" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div>
				<!-- Form Setting -->
				...
				<div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_PSCD_S" name="P_ESTM_PSCD_S" selectKey="${param.P_ESTM_PSCD_S}" cdId="ESTM_PSCD"  headerKey="" cond1="ESTM_PROG" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div>
				<!-- Form Setting -->
			...
			<div class="ui-setting">			
				...
				<div class="form-setting">
					...
					<div class="component-input-search">
						<input type="text" id="P_ESTM_CHRG_DEPT_NM_S" name="P_ESTM_CHRG_DEPT_NM_S" value="${comFn:nvl(param.P_ESTM_CHRG_DEPT_NM_S, P_ESTM_CHRG_DEPT_NM_S)}" class="component-input" readonly="readonly">
						<input type="hidden" id="P_ESTM_CHRG_DEPT_NO_S" name="P_ESTM_CHRG_DEPT_NO_S" value="${comFn:nvl(param.P_ESTM_CHRG_DEPT_NO_S, P_ESTM_CHRG_DEPT_NO_S)}">
		...
		<div class="area-list">
			...
			<div class="table-option">
				<!-- Left -->
				<div class="option-left">
					<button type="button" class="component-button-s" id="registBtn">평가작성</button>
				</div>
				<!-- //Left -->
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(estmProgListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/estm/estmProgListExcelDwld.do','${comFn:nvl(estmProgListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<div style="overflow-x: scroll; overflow-y:hidden" id="list" >
				<table class="component-table">
					...
					<tbody>
						...
						<!-- //엑셀다운로드데이터 종료 -->
						<c:if test="${comFn:nvl(estmProgListTotCnt, 0) == 0}">
							<tr>
								<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						
						<c:if test="${estmProgListTotCnt > 0}">
							<c:forEach var="data" items="${estmProgList}" varStatus="status">
								<tr onclick="detailInqire('${data.ESTM_NO }');">
									...
									<td class="txt-center">
										<c:if test="${data.ESTM_PSCD eq 'A001' }"><span class="tag-status type-ing">${data.ESTM_PSCD_NM}</span></c:if>
										<c:if test="${data.ESTM_PSCD ne 'A001' }"><span class="tag-status type-change">${data.ESTM_PSCD_NM}</span></c:if>
									</td>
									<td>${data.ESTM_SECD_NM }</td>
									<td class="txt-center">${data.ESTM_NO }</td>
									<td title="${data.ESTM_NM }">${comFn:toShorten(data.ESTM_NM, 22)}</td>
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(estmProgListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
