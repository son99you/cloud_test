...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/prdprcRegReqList.js"></script> 
				...
				<div class="view_area">
					<table>
						...
						<tr>
							...
							<th>예가등록상태</th>
							<td>
								<comTag:comCmcdCdValueComboBox name="P_ESTC_PSCD_S" cdId="ESTC_PSCD" selectKey="${param.P_ESTC_PSCD_S }" cond1="1" headerValue="전체" width="180"/>
							</td>	
						</tr>
					</table>				
				</div>
		            	...
		            	<tbody>
			            	<c:if test="${comFn:nvl(prdprcManageListTotcnt,0) > 0}">
				            	<c:forEach var="data" items="${prdprcManageList}" varStatus="status">
					            	<tr class="row" onclick="prvstlPrdprcManageRegistForm('${data.ANNC_NO}', '${data.ANNC_NGR}','${data.ROUND_NO}');" style="cursor: pointer;">
					            		<td>${data.RNUM}</td>
					            		...
					            		<td class="pl5 txtl">${data.SBID_MTCD_NM}</td>
					            		<td>${data.CHRGR_NM}</td>
