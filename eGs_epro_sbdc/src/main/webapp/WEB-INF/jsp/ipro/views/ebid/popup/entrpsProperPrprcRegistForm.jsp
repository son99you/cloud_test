 ...
 * ebid 
 *    |_ popup
 			  |_ entrpsProperPrprcRegistForm.jsp
 * 
 * </pre>
 * @date : 2017. 06. 14
 * @version : 1.0
	...
	<div class="pop_container">
		<div class="view_wrap typeA">
				...
				<form id="registFrm" method="POST">
					...
					<table class="table">
				    	...
				    	<tr>
				        	<th>평가결과</th>
				            <td>
				            	<comTag:cmmnCdValueRadio id="P_ESTM_ELCD" name="P_ESTM_ELCD"  selectKey="${comFn:nvl(entrpsBidInfoInqire.ESTM_ELCD, 'NT_ELGB')}" list="{'ELGB':'적격', 'NT_ELGB':'부적격'}"/>
				            </td>
				        </tr>
				        ...
				        <c:if test="${entrpsBidInfoInqire.SBID_MTCD ne '34'}"><!-- 적격심사 -->
					    	<tr>
					        	<th>평가점수</th>
					            <td>
					            	<input class="w150" name="P_ESTM_SCR" value="${entrpsBidInfoInqire.ESTM_SCR}" numeric>
					            </td>
					        </tr>
				        </c:if>
				        <tr>
				            <th id="ESTM_NT_ELGB_RSN">부적격사유</th>
				            <td>
				            	<textarea id="P_ESTM_NT_ELGB_RSN" name="P_ESTM_NT_ELGB_RSN" maxlength="4000" style="width: 100%; height: 50px;">${entrpsBidInfoInqire.ESTM_NT_ELGB_RSN}</textarea>
				            </td>
				        </tr>
				        <tr>
				            <th class="bullet_orange">검토결과첨부</th>
				            <td>
			...
			<div class="btn_wrap view_btn">
				<c:if test="${entrpsBidInfoInqire.SBID_MTCD eq '31'}">
					<c:if test="${entrpsBidInfoInqire.BID_PSCD eq 'PF50'}">
						<button type="button" class="btn btn_m btn_orange" id="saveBtn">저장</button>
					</c:if>				
				</c:if>
				<c:if test="${entrpsBidInfoInqire.SBID_MTCD ne '31'}">
					<c:if test="${entrpsBidInfoInqire.BID_PSCD eq 'PF20' or entrpsBidInfoInqire.BID_PSCD eq 'PF30'}">
						<button type="button" class="btn btn_m btn_orange" id="saveBtn">저장</button>
					</c:if>
				</c:if>
