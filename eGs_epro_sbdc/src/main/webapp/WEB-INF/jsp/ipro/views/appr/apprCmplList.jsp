...
<script type="text/javascript" src="${jsPath}/ipro/views/appr/apprCmplList.js"></script>
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">결제완료 목록</li>
				</ul>
			</div>
...
<div class="page-list">
	<form id="listFrm" class="search_form" method="POST" action="${contextPath}/info/infoAppResultList.do">
			...
			<div class="table-option">
				...
				<div class="option-right">
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(apprCmplListTotCnt, 0)}</strong>건
					</div>
				</div>
				<!-- //Right -->
			</div>
			...
			<table class="component-table">
				...
				<thead>
					<tr>
						...
						<th class="txt-center">상태</th>
					</tr>
				</thead>
				<tbody>
					<c:if test="${comFn:nvl(apprCmplListTotCnt, 0) == 0}">
						<tr>
							<td colspan="5" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${apprCmplListTotCnt > 0}">
						<c:forEach var="data" items="${apprCmplList}" varStatus="status">
							 <tr onclick="apprCmplDetail('${data.APPR_NO}');">
								<td class="txt-center">${data.APPR_NO}</td>
								<td>${data.APPR_NM}</td>
								<td class="txt-center">${comFn:formatDate(data.REG_DT, 'yyyyMMddhhmmss', 'yyyy-MM-dd')}</td>
								<td class="txt-center">${data.REGR_NM}</td>
								<td class="txt-center">
									<c:if test="${data.APPR_STCD eq 'RTN'}"><span class="tag-status type-change">반려</span></c:if>
									<c:if test="${data.APPR_STCD eq 'DONE'}"><span class="tag-status type-complete">결재완료</span></c:if>
									<c:if test="${data.APPR_STCD eq 'APP'}"><span class="tag-status type-ing">결재중</span></c:if>
								</td>
							</tr>
						</c:forEach>
					</c:if>
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(apprCmplListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
