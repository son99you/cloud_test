...
<div class="area-detail">
	<form id="detailFrm" name="detailFrm" method="POST">
		...
		<div class="table-detail">
			...
			<table class="component-detail-table type-line-none" style="display: none;">
				...
				<tbody>		
					...
					<tr>
						<th>지정평가위원수</th>
						<td colspan="3"><c:if test="${not empty estmMngMstDetail.FIX_ESTM_CMTM_CNT }">${estmMngMstDetail.FIX_ESTM_CMTM_CNT } 명</c:if></td>
					</tr>
				</tbody>
			</table>
		</div>
			...
			<div class="top-detail">
				...
				<div class="type-fright">
						...
						<c:if test="${(estmMngMstDetail.ESTM_PSCD eq 'A004') or (estmMngMstDetail.ESTM_PSCD eq 'A0041')}">
							<c:choose>
								<c:when test="${(P_ESTM_CMTM_RE_SLCT_NGR > 1 ) and (param.P_PRIO_RNK_RE_SLCT_YN eq 'Y') }">
									<a href="javascript:" class="component-button-s type-a" id="estmCmtmReAutoSlctBtn" onclick="estmCmtmReAutoSlct('OUT', '1')">우선순위재선정-자동선별</a>
								</c:when>
								
								<c:when test="${cmtmAutoBtnCtr.OUT_AUTO_BTN_CNTR eq 'F' }">&nbsp;</c:when>
								
								<c:when test="${cmtmAutoBtnCtr.OUT_AUTO_BTN_CNTR eq 'T' }">
									<a href="javascript:" class="component-button-s type-a" id="estmCmtmReAutoSlctBtn" onclick="estmCmtmReAutoSlct('OUT', '2')">우선순위재선정-자동선별</a>
								</c:when>
					...
					<c:if test="${estmMngMstDetail.CMTM_SLCT_CMPL_YN ne 'Y'}">
						<c:if test="${(estmMngMstDetail.ESTM_PSCD ne 'A003') and (estmMngMstDetail.ESTM_PSCD ne 'A004') and (estmMngMstDetail.ESTM_PSCD ne 'A005') and (estmMngMstDetail.ESTM_PSCD ne 'A0041') }">
							<c:if test="${ estmMngMstDetail.OUT_CMTM_SLCT_MTHD_SECD eq 'A' }">
								<!-- 자체선정일 경우 -->
								<a href="javascript:" class="component-button-s type-a" onclick="estmCmtmRegist('OUT','outShowTbdy')">평가위원저장</a>
						...
						<c:if test="${not empty outEstmCmtmList}">
							<c:forEach var="data" items="${outEstmCmtmList}" varStatus="status">
								<tr>
									...
									<td class="txt-center">
										${data.ESTM_CMTM_NM }
										<input type="hidden" name="P_ESTM_CMTM_NM" value="${data.ESTM_CMTM_NM }" class="component-input" disabled="disabled">
									...
									<c:if test="${(estmMngMstDetail.CMTM_SLCT_CMPL_YN eq 'Y') and (param.P_CMPL_YN eq 'Y')}"><!-- 최종 -->
										...
										<td class="txt-center">
											<label class="component-checkbox">
												<input type="checkbox" name="hldfYnCbk" <c:if test="${data.HLDF_YN eq 'Y'}">checked</c:if> onclick="hldfYnCbkEvent(this);">
												<i></i>
											</label>
											<input type="hidden" name="P_HLDF_YN" value="${data.HLDF_YN }">
										</td>
									</c:if>
								</tr>
							</c:forEach>
			...
			<div class="table-detail">
				...
				<div class="top-detail">
					...
					<c:if test="${estmMngMstDetail.CMTM_SLCT_CMPL_YN ne 'Y'}">
						<c:if test="${(estmMngMstDetail.ESTM_PSCD ne 'A003') and (estmMngMstDetail.ESTM_PSCD ne 'A004') and (estmMngMstDetail.ESTM_PSCD ne 'A005') and (estmMngMstDetail.ESTM_PSCD ne 'A0041') }">
							<div class="type-fright">
								<a href="javascript:" class="component-button-s type-a" onclick="estmCmtmRegist('OUT','fixShowTbdy')">평가위원저장</a>
		...
		<%-- 선정된 외부평가위원 수 ==> ${ estmMngMstDetail.OUT_CMTM_SLCT_CNT}
		<br>
		뽑아야하는 외부평가위원 수 ==> ${ estmMngMstDetail.OUT_ESTM_CMTM_CNT}
		<br>*************************************<br>
		...
		<div class="bottom-buttons">
			<c:choose>
				...
				<c:when test="${(estmMngMstDetail.OUT_CMTM_SLCT_CNT) eq (comFn:nvl(estmMngMstDetail.OUT_ESTM_CMTM_CNT, 0) )
					and (estmMngMstDetail.INN_CMTM_SLCT_CNT) eq (comFn:nvl(estmMngMstDetail.INN_ESTM_CMTM_CNT, 0) ) }">
						<!-- 선정된 평가위원 = 평가위원 -->
						<!-- 외부평가위원선정방법 : 자체선정 or 내부평가위원선정방법 : 자체선정 -->
			...
			<c:if test="${estmMngMstDetail.OUT_CMTM_SLCT_MTHD_SECD ne 'Z'}">
				<c:if test="${(estmMngMstDetail.CMTM_SLCT_CMPL_YN eq 'Y') and (param.P_CMPL_YN eq 'Y')}">
					<c:if test="${estmMngMstDetail.ESTM_PSCD ne 'A005'}"><!-- 평가시작 -->
						<a href="javascript:" class="btn-bottom type-b" id="rsdnNoSaveBtn">주민등록번호 저장</a>
					</c:if>
				</c:if>
			</c:if>
			
			<c:if test="${(estmMngMstDetail.ESTM_PSCD eq 'A005')}"><!-- 평가시작 -->
				<a href="javascript:" class="btn-bottom type-b" id="hldfYnUpdtBtn">재직여부 저장</a>
			</c:if>
...
<!-- 선정된 평가위원 주민등록번호/재직여부 저장을 위한 FORM -->
