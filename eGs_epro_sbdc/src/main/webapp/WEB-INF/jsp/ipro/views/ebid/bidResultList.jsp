...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bidResultList.js"></script> 
		...
		<fieldset>
				...
				<div class="view_area">
					<table>
						...
						<tr>
							...
							<th>진행상태</th>
							<td>
								<comTag:comCmcdCdValueComboBox name="P_BID_PSCD_S" cdId="BID_PSCD" cond1="RESULT" selectKey="${param.P_BID_PSCD_S }" headerValue="전체" width="180"/>
							</td> 
						</tr>
						<tr>
							<th>입찰담당자</th>
			...
			<div class="list_wrap mt30" id="contentWrap">
					...
					<table>
				        ...
				        <tbody>
							<c:if test="${comFn:nvl(bidResultListTotcnt, 0) == 0}">
								<tr>
									<td colspan="9" class="txtc">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
								</tr>
							</c:if>
							<c:if test="${bidResultListTotcnt > 0}">
								<c:forEach var="data" items="${bidResultList}" varStatus="status">
									<tr>
										...
										<td class="pl5 list_tit left_T" title="${ data.BID_NM}" onclick="bidResultDetail('${data.ANNC_NO}','${data.ANNC_NGR }','${data.ROUND_NO }','${data.ANNC_SECD }');"  style="cursor: pointer;">					
											<c:if test="${data.EMRG_YN eq 'Y'}"><font color="red">[긴급] </font></c:if>
											<c:if test="${data.ANNC_NGR > 1}"><font color="red">[정정] </font></c:if>
											${comFn:toShorten(data.BID_NM,30)}
										</td>
				...
				<div class="list_bottom">
					<comTag:pagingIpro totalCount="${bidResultListTotcnt}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
				</div> <!--// list_bottom E -->			
		    </div>				
