...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/estmCmplList.js"></script>
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">평가완료현황 목록</li>
				</ul>
			</div>
...
<div class="page-list">
	<form id="searchFrm" name="searchFrm" method="post">
		...
		<div class="form-setting-box">
			...
			<div class="ui-setting">
				...
				<div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_PSCD_S" name="P_ESTM_PSCD_S" selectKey="${param.P_ESTM_PSCD_S}" cdId="ESTM_PSCD"  headerKey="" cond1="ESTM_CMPL" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div>
				<!-- Form Setting -->
		...
		<div class="area-list">
			...
			<div class="table-option">
				...
				<div class="option-right">
					...
					<a href="javascript:excelDwd('searchFrm', '/estm/estmCmplListExcelDwld.do','${comFn:nvl(estmCmplListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<table class="component-table" >
				...
				<thead id="excelTh">
					<tr>
						...
						<!-- <th class="txt-center" style="display: none;">작성자2</th> -->
					</tr>
				</thead>
				<tbody>
					<!-- 엑셀다운로드데이터 시작 -->
					...
					<!-- //엑셀다운로드데이터 종료 -->
					<c:if test="${comFn:nvl(estmCmplListTotCnt, 0) == 0}">
						<tr>
							<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${estmCmplListTotCnt > 0}">
						<c:forEach var="data" items="${estmCmplList}" varStatus="status">
							<tr onclick="detailInqire('${data.ESTM_NO }');">
								...
								<td title="${data.ESTM_NM }">${comFn:toShorten(data.ESTM_NM, 18)}</td>
								<td>${comFn:formatDate(data.TOTL_ESTM_ST_DT,'yyyyMMddHHmmss','yyyy-MM-dd HH:mm:ss')} <br> ~ ${comFn:formatDate(data.TOTL_ESTM_END_DT,'yyyyMMddHHmmss','yyyy-MM-dd HH:mm:ss')}</td>
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(estmCmplListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
	</form>	
</div>
