...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/opengManageList.js"></script> 
...
<div class="contents_wrap">
	...
	<div class="list_wrap mt30" id="contentWrap">
		<div class="list_top">
			<p class="total">총 <span>${comFn:nvl(opengManageListTotcnt, 0)}</span>건</p>		
		</div> <!--// list_top E -->
		<div class="list_conts">		
			<table>
				<caption>개찰대기현황 목록</caption>
              		<colgroup>
                   	<col width="8%"/>
                   	<col width="12%"/>
                   	<col width="*"/>
            	...
            	<thead>
			    	<tr>
			            ...
			            <th scope="col">투찰마감일시</th>
			            <th scope="col">투찰업체수</th>
			            <th scope="col" class="last">열람</th>
			        </tr>
            	</thead>
            	<tbody>
			        <c:if test="${comFn:nvl(opengManageListTotcnt, 0) == 0}">
						<tr>
							<td colspan="8" class="txtc">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${opengManageListTotcnt > 0}">
						<c:forEach var="data" items="${opengManageList}" varStatus="status">
							<tr class="row">
								<td>${data.CONT_SECD_NM}</td>
								<td>${data.ANNC_NO}-${data.ANNC_NGR}</td>
								<td class="pl5 list_tit" title="${data.BID_NM}" onclick="opengManageDetailInqire('${data.ANNC_NO}', '${data.ANNC_NGR}', '${data.ROUND_NO }');" style="cursor: pointer;">
		...
		<div class="list_bottom">
			<comTag:pagingIpro totalCount="${opengManageListTotcnt}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div> <!--// list_bottom E -->			
    </div>		
