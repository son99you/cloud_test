...
<%--
 * 평가관리 > 평가완료현황_평가위원 상세 - 화상회의
 *
 * <pre>
 * estm
 *    |_ estmCmtmCmplVidoMtngDetail.jsp
 * 
 * </pre>
 * @date : 2021. 03. 22.
 * @version : 1.0
...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/estmCmtmCmplVidoMtngDetail.js"></script>
			...
			<table class="component-detail-table ">
				...
				<tbody>
					<c:if test="${empty estmVidoList}">
						<tr>
							<td colspan="3" class="txt-center">데이터가 존재하지 않습니다</td>
						</tr>
					</c:if>
					<c:if test="${not empty estmVidoList}">
						<c:forEach var="data" items="${estmVidoList}" varStatus="status">
							<tr>
								...
								<td class="txt-center">
									<button type="button" name="mtngPrtcRqs" class="component-button-s type-a">다운로드</button>
								</td>
							</tr>
						</c:forEach>
					</c:if>
