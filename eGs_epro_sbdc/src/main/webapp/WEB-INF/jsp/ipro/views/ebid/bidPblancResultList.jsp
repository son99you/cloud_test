...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bidPblancResultList.js"></script> 
	...
	<!-- 				<a href="#" class="filter_btn">상세검색필터 보기</a> -->
					...
					<dl> 
						...
						<dd style="padding-left: 8px;">
							<comTag:comCmcdCdValueComboBox id="P_BID_PSCD_S" name="P_BID_PSCD_S" selectKey="${param.P_BID_PSCD_S}" cdId="BID_PSCD" headerValue="전체" className="select w100p" cond1="B"/>
						</dd>
					</dl>
				</div><!--// sch-box -->
				...
				<div class="list_top">
					<p class="total">총 <span>${comFn:nvl(bidPblancResultListTotcnt, 0)}</span>건</p>
				...
				<div style="overflow-x: scroll; overflow-y:hidden" >
					<table class="list_tb">
						...
						<tbody>
							<c:if test="${comFn:nvl(bidPblancResultListTotcnt, 0) == 0}">
								<tr>
									<td colspan="11" align="center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
								</tr>
							</c:if> 
							<c:if test="${bidPblancResultListTotcnt > 0}">
								<c:forEach var="data" items="${bidPblancResultList}" varStatus="status" >
									<tr class="row" onclick="detailInqire('${data.PRCH_RQR_NO}','${data.ANNC_NO}','${data.ANNC_NGR}','${data.ROUND_NO}','${data.BID_PSCD}');" style="cursor: pointer;">
										...
										<%-- <td><c:if test="${data.APPR_STCD eq 'N'}" >미결재</c:if><c:if test="${data.APPR_STCD eq 'Y'}" >결재</c:if></td> --%>
									</tr>
								</c:forEach>
							</c:if> 
						</tbody>
				...
				<div class="paging">
					<comTag:pagingIpro totalCount="${comFn:nvl(bidPblancResultListTotcnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
				</div><!--// paging -->
		</div><!--// sp_cont --> 
	</form>
</div><!--// sp_sec -->		
