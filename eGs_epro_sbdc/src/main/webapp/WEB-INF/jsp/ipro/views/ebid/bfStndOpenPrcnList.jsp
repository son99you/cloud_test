...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bfStndOpenPrcnList.js"></script> 
		...
		<input type="hidden" id="P_SEARCH" name="P_SEARCH" value="">
			...
			<div class="sp_cont">
				<div class="sch_box filter_sch">
					...
					<dl>
						<dt>진행상태</dt>
						<dd>
							<comTag:comCmcdCdValueComboBox id="P_BFAN_PSCD_S" name="P_BFAN_PSCD_S" selectKey="${param.P_BFAN_PSCD_S}" cdId="BFAN_PSCD" cond3="Y"  headerValue="전체" className="select w100p"/>
						</dd>
						<dt>요청일자</dt>
						<dd class="date">
				...
				<div class="list_top">
				<p class="total">총 <span>${comFn:nvl(bfStndOpenPrcnListTotCnt, 0)}</span>건</p>
			...
			<div style="overflow-x: scroll; overflow-y:hidden" >
				<table class="list_tb">
			        ...
			        <tbody>
						<c:if test="${comFn:nvl(bfStndOpenPrcnListTotCnt, 0) == 0}">
							<tr class="row">
								<td colspan="13" align="center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						<c:if test="${bfStndOpenPrcnListTotCnt > 0}">
							<c:forEach var="data" items="${bfStndOpenPrcnList}" varStatus="status" >
			...
			<div class="paging">
				<comTag:pagingIpro totalCount="${comFn:nvl(bfStndOpenPrcnListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
			</div> <!--// list_bottom E -->
		</div> <!--// sp_cont -->
	</form>
</div><!--// sp_sec -->		
