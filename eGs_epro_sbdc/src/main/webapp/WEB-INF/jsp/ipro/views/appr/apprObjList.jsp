...
<script type="text/javascript" src="${jsPath}/ipro/views/appr/apprObjList.js"></script>
	...
	<form id="listFrm" class="listFrm" method="POST">
		...
		<div class="area-list">
			...
			<div class="table-option">
				...
				<div class="option-right">
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(apprObjListTotCnt, 0)}</strong>건
					</div>
				</div>
				<!-- //Right -->
			</div>
			...
			<table class="component-table">
				...
				<tbody>
					<c:if test="${comFn:nvl(apprObjListTotCnt, 0) == 0}">
						<tr>
							<td class="txt-center" colspan="4">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${apprObjListTotCnt > 0}">
						<c:forEach var="data" items="${apprObjList}" varStatus="status">
							<tr onclick="apprObjDetail('${data.APPR_NO}');">
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(apprObjListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
