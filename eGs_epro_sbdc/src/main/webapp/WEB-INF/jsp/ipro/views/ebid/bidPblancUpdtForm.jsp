...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bidPblancUpdtForm.js"></script>
			...
			<div class="view_wrap typeA">
				...
				<div class="view_area">
					<table>
		                ...
		                <tr class="grlDiv">
		                    <th scope="row" class="bullet_orange"><label for="P_RMK">정정공고사유</label></th>
		                    <td>
		                        <textarea id="P_PROC_RSN" name="P_PROC_RSN" style="width: 100%; height: 120px;" maxlength="4000"></textarea>
		                    </td>
		                </tr>				    	
				    </table>
				</div>
				...
				<div class="view_area">
					<table>
				    	...
				    	<tr>
				    		<th scope="row"><label for="P_BID_MTCD">입찰방법</label></th>
				    		<td>
				    			${bidPlanDetail.BID_MTCD_NM}
				    		</td>
				            <th scope="row"><label for="P_CONT_SECD">계약구분</label></th>
				            <td>
				...
				<div class="view_area">
					<table>
				        ...
				        <tr>
				            <th scope="row" class="bullet_orange bullet_orange2"><label for="P_CONT_MTCD">계약방법</label></th>
				            <td colspan="3">
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'EST'}">
				            		<comTag:comCmcdCdValueComboBox id="P_CONT_MTCD" name="P_CONT_MTCD" selectKey="${bidPlanDetail.CONT_MTCD}" cdId="CONT_MTCD" cond1="EST" headerKey="" headerValue="선택" width="180"/>
				            	</c:if>
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'BID'}">
				            	...
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'PRVT'}">
					            	<comTag:comCmcdCdValueComboBox id="P_CONT_MTCD" name="P_CONT_MTCD" selectKey="${bidPlanDetail.CONT_MTCD}" cdId="CONT_MTCD" cond3="PRVT" headerKey="" headerValue="선택" width="160"/>
				            	</c:if>
				            </td>
				        </tr>
				        <tr>
				        	<th scope="row" class="bullet_orange bullet_orange2"><label for="P_SBID_MTCD">낙찰자선정방법</label></th>
				            <td>
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'EST'}">
				            		<comTag:comCmcdCdValueComboBox id="P_SBID_MTCD" name="P_SBID_MTCD" selectKey="${bidPlanDetail.SBID_MTCD}" cdId="SBID_MTCD" cond3="EST" headerKey="" headerValue="선택" width="160"/>
				            	</c:if>
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'BID'}">
				            	...
				            	<c:if test="${bidPlanDetail.BID_MTCD eq 'PRVT'}">
				            		수의계약
				            		<input type="hidden" name="P_SBID_MTCD" value="55">
				            	</c:if>
				            </td>
							<th scope="row" id="negoTH">
								<%-- 낙찰자선정방식이 [적격심사]가 아닐 경우 활성화 --%>
								<label for="P_SBID_LWST_RT" class="elgbDiv grlDiv lwstDiv" <c:if test="${bidPlanDetail.SBID_MTCD ne '31'}">style="display: none;"</c:if>>낙찰하한율 (%)</label>
...
<%-- 			                        	<comTag:comCmcdCdValueComboBox id="P_SUM_RTCD" name="P_SUM_RTCD"   selectKey="${bidPlanDetail.SUM_RTCD}" cdId="SUM_RTCD" headerKey="" headerValue="선택" className="negoDiv" width="160"/> --%>
				        ...
				        <tr class="prepDiv grlDiv" <c:if test="${bidPlanDetail.ESTPC_SECD ne '180000'}"> style="display:none;"</c:if>>
				            ...
				            <td colspan="3">
			            		<comTag:comCmcdCdValueComboBox id="P_PLR_ESTPC_RNG_CD" name="P_PLR_ESTPC_RNG_CD"   selectKey="${comFn:nvl(bidPlanDetail.PLR_ESTPC_RNG_CD,'NTN')}" cdId="PLR_ESTPC_RNG_CD" headerKey="" headerValue="선택" width="180"/>
				        ...
				        <tr class="grlDiv">    
				            ...
				            <%-- 입찰설명회 [예] 일 경우 활성화 --%>
				            <th scope="row">
				            	<label class="brfsDiv" <c:if test="${bidPlanDetail.BID_BRFS_YN ne 'Y'}">style="display:none;"</c:if>>입찰설명회필수참석여부</label>
				            </th>
				            <td>
						...
						<tr class="exmtNDiv grlDiv">
		                    ...
		                    <td>
		                    	<input type="text" id="P_BID_GTAMT_RT" name="P_BID_GTAMT_RT" value="${bidPlanDetail.BID_GTAMT_RT }" class="PT2 bidgrRtDiv w180">
		                    </td>
		                    <th scope="row"><label for="P_BIDGR_AMT" class="bidgrAmtDiv">입찰보증금 (원)</label></th>
		                    <td>
								<input type="text" id="P_BIDGR_AMT" name="P_BIDGR_AMT" value="${comFn:formatMoney(bidPlanDetail.BIDGR_AMT) }" money class="bidgrAmtDiv w180">
		                    </td>
		                </tr>
		                <tr class="grlDiv">
			            ...
			            <table>
			                ...
			                <tbody id="nmenChoiseHiddTbdy">
			                	<tr style="display: none;">
									...
									<td entrpsNm></td>
									<td rprsntvNm style="text-align: center;"></td>
								</tr>
			                </tbody>
				...
				<div class="view_area grlDiv">
		            <table>
		                ...
		                <tr>
		                    ...
		                    <td>
		                        <input type="text" class="tr PT2 w180" id="P_CPDF_RT" name="P_CPDF_RT" value="<c:if test="${empty bidPlanDetail.ANNC_NO}">0.25</c:if><c:if test="${not empty bidPlanDetail.ANNC_NO}">${bidPlanDetail.CPDF_RT}</c:if>" numeric maxlength="4"> /1,000
		                    </td>
		                ...
		                <tr>
		                    ...
		                    <td>
		                        <input type="text" class="tr PT2 w180" id="P_DFFL_GTAMT_RT" name="P_DFFL_GTAMT_RT" value="<c:if test="${empty bidPlanDetail.ANNC_NO}">2</c:if><c:if test="${not empty bidPlanDetail.ANNC_NO}">${bidPlanDetail.DFFL_GTAMT_RT}</c:if>" numeric maxlength="4">
		                    </td>
		                    <%-- 계약구분이 [구매] 일 경우 활성화 --%>
			            ...
			            <table style="width:100%;">
			                ...
			                <tbody id="biprInfoShowTbdy">
			                	<c:forEach var="data" items="${bidPrdlsList}" varStatus="status">
									<tr class="row">
										...
										<td class="txtc"><input type="hidden" name="P_ITEM_UNNM" value="${data.ITEM_UNNM}">${data.ITEM_UNNM}</td>
										...
										<td class="txtr pr5">
											${comFn:formatMoney(data.RQST_UNIT)}
											<input type="hidden" name="P_SCH_UNIT" value="${data.RQST_UNIT}">
										</td>
									</tr>
								</c:forEach>
			                </tbody>
			                <tbody>
								<tr class="row" id="biprInfoEmpty" <c:if test="${ not empty bidPrdlsList}"> style="display:none;"</c:if>>
				               		<td colspan="8" class="txtc">선택된 입찰품목이 없습니다.</td>
				               	</tr>
			               	</tbody>
			            </table>
			        </div>
...
<form id="listFrm" method="POST">
	<input type="hidden" name="resourceName" value="${param.resourceName}">
  	<input type="hidden" id="P_PAGE_NO_LIST" name="P_PAGE_NO" value="1">
	<input type="hidden" id="P_PAGE_SIZE_LIST" name="P_PAGE_SIZE" value="10">
</form>
...
<form id="popupFrm" method="POST">
	...
	<input type="hidden" name="P_USER_NM_S">
	<input type="hidden" name="P_ORG_NM_S" value="기술평가팀">
</form>
<form id="bidClientPopupFrm" method="POST">
	<input type="hidden" name="P_USER_NM_S">
	<input type="hidden" name="P_ORG_NM_S">
	<input type="hidden" name="searchGbnId" value="bidClientTd">
</form>
