...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/cmtmRnkSlctCmplDetail.js"></script>
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">평가위원순위선정완료현황 상세</li>
				</ul>
			</div>
...
<div class="area-detail">
	<form id="detailFrm" name="detailFrm" method="POST">
		...
		<input type="hidden" id="P_ESTM_CMTM_SLCT_NGR" name="P_ESTM_CMTM_SLCT_NGR" value="${P_ESTM_CMTM_SLCT_NGR}">
		...
		<div class="table-detail">
			...
			<div class="type-fleft">
				<c:if test="${comFn:nvl(estmMngMstDetail.MAX_ESTM_CMTM_SLCT_NGR, 1) > 0 }">
					<c:forEach begin="1" end="${ comFn:nvl(estmMngMstDetail.MAX_ESTM_CMTM_SLCT_NGR, 1) }" step="1" var="estmCmtmSlctNgr" varStatus="loop">
						...
						<c:if test="${ estmCmtmSlctNgr ne P_ESTM_CMTM_SLCT_NGR }">
			...
			<c:if test="${ (estmMngMstDetail.OUT_CMTM_SLCT_MTHD_SECD eq 'B') or (estmMngMstDetail.OUT_CMTM_SLCT_MTHD_SECD eq 'C') }">
				...
				<table class="component-detail-table ">
					...
					<tbody>
						<c:if test="${estmMngMstDetail.ESTM_PSCD ne 'A003' }">
