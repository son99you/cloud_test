...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/popup/bidAssrncInfoDetail.js"></script> 
 
<div class="pop_wrap">
	<div class="pop_header">
		<h1 class="tit">보증정보조회</h1>
	</div>
	<div class="pop_container">
		<div class="view_wrap typeC">
			<div class="view_area">
				<table>
			    	...
			    	<tr class="line">
		               ...
		               <th>공고일자</th>
		              	<td>${comFn:formatDate(bidAssrncInfoInqire.ANNC_DT,'yyyyMMddHHmmss','yyyy-MM-dd')}</td>	     
		           </tr>
		           <tr>
		               <th>공고명</th>
			...
			<!-- 입찰보증구분이 보증증권일 경우 -->
			<c:if test="${entrpsGrntyInfoList[0].BIDGR_SECD eq 'GRNT_STCK'}">
				<div class="view_area typeB">
		    		<table id="tableList">
						...
						<tbody>
							<c:if test="${not empty entrpsGrntyInfoList}">
								<c:forEach var="data" items="${entrpsGrntyInfoList}" varStatus="status">
									<tr>
										<td class="txtc">${data.BIDGR_SECD_NM }</td>
										<td class="txtr pr5">${comFn:formatMoney(data.BIDGR_AMT) }</td>
										<td>${data.BIDGR_NO }</td>
										<td>${data.BIDGR_AGNM }</td>
									</tr>
					        	</c:forEach>
							</c:if>
							<c:if test="${empty entrpsGrntyInfoList}">
								<tr>
									<td colspan="4" class="txtc">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
								</tr>
							</c:if>
		      			</tbody>
					</table>
