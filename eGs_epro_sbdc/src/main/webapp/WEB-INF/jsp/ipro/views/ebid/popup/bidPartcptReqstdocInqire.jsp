...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/popup/bidPartcptReqstdocInqire.js"></script> 
 
<div class="pop_wrap">
	<div class="pop_header">
		<h1 class="tit">
			<c:if test="${param.P_BID_TPI_SECD eq 'OP01'}">
				<c:if test="${param.P_BID_GBN eq 'EST' }">견적참가서류</c:if>
				<c:if test="${param.P_BID_GBN ne 'EST'}">입찰참가서류</c:if>
			</c:if>	
			<c:if test="${param.P_BID_TPI_SECD eq 'OP05'}">제안서/규격서 조회</c:if>
		</h1>
	</div> <!--// pop_header E -->
	<div class="pop_container">
		<div class="view_wrap typeC">
			<div class="tit_area">
				<h2 class="tit01">개요</h2>
			</div>
			<div class="view_area">
				<table class="table">
			    	<colgroup>
    		...
    		<div class="tit_area">
				<h2 class="tit01">
					<c:if test="${param.P_BID_TPI_SECD eq 'OP01'}">입찰관련문서</c:if>
					<c:if test="${param.P_BID_TPI_SECD eq 'OP05'}">제안/규격서</c:if>
				</h2>
			</div>
