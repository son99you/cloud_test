...
<script type="text/javascript" src="${jsPath}/ipro/views/ebid/bidDcPeoList.js"></script>
...
<div class="contents_wrap">
				...
				<div class="view_area">
					<table>
						...
						<tr>
							...
							<th>낙찰방법</th>
							<td>
								<comTag:comCmcdCdValueComboBox name="P_SBID_MTCD_S" cdId="SBID_MTCD" selectKey="${param.P_SBID_MTCD_S }" headerValue="전체" width="180"/>
							</td> 						
						</tr>
						<tr>
							<th>설명회일자</th>
							<td>
								<div class="calendar_box">
				                    <label for="P_BRFS_STDT_S" class="blind">설명회일자 시작일</label>
				                    <input type="text" class="datepicker1 w120" id="P_BRFS_STDT_S" name="P_BRFS_STDT_S" value="${param.P_BRFS_STDT_S}" date >
				                	<span class="wave"> ~ </span>
				                    <label for="P_BRFS_ENDT_S" class="blind">설명회일자 마감일</label>
				                    <input type="text" class="datepicker2 w120" id="P_BRFS_ENDT_S" name="P_BRFS_ENDT_S" value="${param.P_BRFS_ENDT_S}" date >
				                </div>
							</td>
							<th>입찰담당자</th>
							<td colspan="3">
	...
	<div class="list_wrap mt30" id="contentWrap">
		<div class="list_top">
			<p class="total">총 <span>${comFn:nvl(bidDcPeoListTotcnt, 0)}</span>건</p>		
		</div> <!--// list_top E -->
		<div class="list_conts" style="overflow: auto;">
			<table style="width:130%;">
				<caption>현장설명회 목록</caption>
              		<colgroup>
                   	<col width="4%"/>
                   	<col width="6%"/>
                   	<col width="8%"/>
            	...
            	<thead>
			    	<tr>
			            ...
			            <th scope="col">공고명</th>
			            <th scope="col">설명회일시</th>
			            <th scope="col">계약방법</th>
			            <th scope="col">낙찰방법</th>
			            <th scope="col">입찰담당자</th>
			            <th scope="col">참석업체수</th>
			            <th scope="col" class="last">열람</th>
			        </tr>
            	</thead>
		        <tbody>
					<c:if test="${comFn:nvl(bidDcPeoListTotcnt, 0) == 0}">
						<tr>
							<td colspan="11" class="txtc">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${bidDcPeoListTotcnt > 0}">
						<c:forEach var="data" items="${bidDcPeoList}" varStatus="status">
							<tr class="row">
								<td>${data.RNUM}</td>
								<td>${data.CONT_SECD_NM}</td>
								<td>${data.ANNC_NO}-${data.ANNC_NGR}</td>
								<td class="pl5 list_tit" title="${data.BID_NM}" onclick="bidDcPeoDetail('${data.ANNC_NO}','${data.ANNC_NGR }','${data.ROUND_NO }');" style="cursor: pointer;">
									<c:if test="${data.EMRG_YN eq 'Y'}"><font color="red">[긴급] </font></c:if>
									<c:if test="${data.ANNC_NGR > 1}"><font color="red">[정정] </font></c:if>
									${comFn:toShorten(data.BID_NM, 70)}</td>
								<td>${comFn:formatDate(data.BRFS_DT,'yyyyMMddHHmmss','yyyy-MM-dd HH:mm')}</td>									
								<td>${data.CONT_MTCD_NM}</td>
								<td>${data.SBID_MTCD_NM}</td>
								<td>${data.CHRGR_NM}</td>
								<td>${data.ATND_VEND_CNT }</td>
		...
		<div class="list_bottom">
			<comTag:pagingIpro totalCount="${bidDcPeoListTotcnt}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div> <!--// list_bottom E -->			
    </div>		
