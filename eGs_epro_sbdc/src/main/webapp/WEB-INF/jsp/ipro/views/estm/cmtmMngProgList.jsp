...
<%--
 * 평가관리 > 평가위원관리 > 평가위원평가진행현황 목록
 *
 * <pre>
 * estm
 *    |_ cmtmMngProgList.jsp
 * 
 * </pre>
 * @date : 2021. 03. 23.
 * @version : 1.0
...
<script type="text/javascript" src="${jsPath}/ipro/views/estm/cmtmMngProgList.js"></script> 
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">평가위원평가진행현황 목록</li>
				</ul>
			</div>
...
<div class="page-list">
	<form id="searchFrm" name="searchFrm" method="post">
		...
		<div class="form-setting-box">
			...
			<div class="ui-setting">
				...
				<div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_SECD_S" name="P_ESTM_SECD_S" selectKey="${param.P_ESTM_SECD_S}" cdId="ESTM_SECD"  headerKey="" cond1="CMTM_PROG" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div>
				<!-- Form Setting -->
			...
			<div class="table-option">
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(cmtmMngProgListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/estm/cmtmMngProgListExcelDwld.do','${comFn:nvl(cmtmMngProgListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<div style="overflow-x: scroll; overflow-y:hidden" >
				<table class="component-table">
					...
					<tbody>
						...
						<c:if test="${comFn:nvl(cmtmMngProgListTotCnt, 0) == 0}">
							<tr>
								<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						
						<c:if test="${cmtmMngProgListTotCnt > 0}">
							<c:forEach var="data" items="${cmtmMngProgList}" varStatus="status">
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(cmtmMngProgListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
