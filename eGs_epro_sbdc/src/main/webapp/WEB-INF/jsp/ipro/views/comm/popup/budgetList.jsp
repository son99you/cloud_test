...
<script type="text/javascript" src="${jsPath}/ipro/views/comm/popup/budgetList.js"></script>
<div class="pop_sp_sec"> 
	...
	<div class="sp_cont">
		<form id="searchFrm" class="search_form" method="POST" >
			...
			<input type="hidden" id="P_BDG_DDTL_ID_S" name="P_BDG_DDTL_ID_S" value="${param.P_BDG_DDTL_ID_S}">
			...
			<tbody>
				...
				<c:if test="${budgetInqireListTotcnt > 0}">
					<c:forEach var="data" items="${budgetInqireList}" varStatus="status" >
						<tr class="row" <c:if test="${param.setMulti ne 'Y'}">onclick="setbudgetInfo('${data.DEPT_NM}','${data.DEPT_NO }','${data.BSNS_CD}','${data.BSNS_NM }','${data.ACNT_CD}','${data.ACNT_NM }','${data.ACNT_ITEM_CD }','${data.RQST_BLNC_AMT}', '${data.BDG_AMT}', '${data.EXPS_RQST_AMT}', '${data.USE_BDG_AMT}', '${data.BDG_YR}', '${data.BDG_MM}', '${data.DDTLBZ_ID}');"</c:if> style="cursor: pointer;">
							...
							<td  class="txtr pr5">
								...
								<input type="hidden" name="P_DDTLBZ_ID" value="${data.DDTLBZ_ID}">
							</td>
							<td >
		...
		<!-- Data Paging -->
		<div class="paging">
			<comTag:pagingIpro totalCount="${budgetInqireListTotcnt}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
