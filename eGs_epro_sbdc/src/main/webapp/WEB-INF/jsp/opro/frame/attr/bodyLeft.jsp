...
<script type="text/javascript" src="${jsPath}/opro/views/comm/frame/bodyLeft.js"></script>  

<input type="hidden" id="P_USR_SECD" name="P_USR_SECD" value="${P_USR_SECD }">
<div class="page-view">
	<!-- Lnb -->
	<div class="sub-lnb">
		...
		<c:if test="${not empty myMenuSubList}">
			...
			<c:forEach var="menuSubList" items="${myMenuSubList.myMenuSubList}" varStatus="idx">
				...
				<c:if test="${menuSubList.MENU_SECD eq 'P'}">
					<li>
						<a href="#" onclick="clickMenuMove('${menuSubList.LNK_URL}', '${menuSubList.MENU_ID}', '${menuSubList.MENU_NM}' );" title="${menuSubList.MENU_NM}">${menuSubList.MENU_NM}</a>
					</li>
				</c:if>
...
<form id="menuMoveFrm" method="post">
	<input type="hidden" name="P_EMPL_ID" value="${loginResult.USR_ID}" >
