	                ...
	                <tbody>
				        ...
				        <c:if test="${inProgrsBidPblancDetail.SBID_MTCD ne '70'}">
					        ...
					        <c:if test="${inProgrsBidPblancDetail.ESTPC_SECD eq '180000'}">
						        <tr>
						        	<th>복수예비가격범위</th>
						            <td colspan="3">${inProgrsBidPblancDetail.PLR_ESTPC_RNG_CD_NM}</td>
						        </tr>
					        </c:if>
					        <tr>
								<th>산정기준&산정방법</th>
		...
		복수예가방식을 사용하지 않고 기초금액을 근거로 하여 담당자가 미리 예정가격을 결정하는 방식
						...
						<c:if test="${inProgrsBidPblancDetail.SBID_MTCD ne '70'}">
							<tr <c:if test="${inProgrsBidPblancDetail.EXMT_YN ne 'N'}">style="display: none;"</c:if>>
			                    <th>입찰보증률</th>
			                    <td>${inProgrsBidPblancDetail.BID_GTAMT_RT}</td>
			                    <th>입찰보증금</th>
			                    <td>${comFn:formatMoney(inProgrsBidPblancDetail.BIDGR_AMT)}</td>
			                </tr>
			                <tr>
			                    <th>입찰참가자격</th>
			                    <td colspan="3">
