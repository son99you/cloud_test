...
<script type="text/javascript" src="${jsPath}/opro/views/noti/qnaList.js"></script>
	...
	<div class="table-detail">
		...
		<div class="nav_sec">
			...
			<div class="option-left">
				<ul class="location">
					<li style="font-size: 30px; font-weight: 500;">QNA 목록</li>
				</ul>
			</div>
	   ...
	   <div class="form-setting-box">
		   ...
		   <div class="table-option">
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(qnaListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/opro/noti/qnaListExcelDwld.do','${comFn:nvl(qnaListTotCnt, 0)}');" class="btn-download-s type-fleft">
		...
		<!-- //Option-->
             	...
             	<colgroup>
           		...
           		<tbody>
					...
					<input type="hidden" name="P_EXCEL_TD" value="INQ_CNT"/>
           			<c:if test="${comFn:nvl(qnaListTotCnt, 0) == 0}">
						<tr>
							<td class="txtc" colspan="5">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${qnaListTotCnt > 0}">
						<c:forEach var="data" items="${qnaList}" varStatus="status" >
							<tr  onclick="detailInqire('${data.BBS_SECD }', '${data.BBS_SN}');" style="cursor: pointer;">
								<td class="txt-center">${data.RNUM}</td>
								<td  title="${data.TTL_NM}">
									<a href="#"><c:if test="${data.RE_LEV > 0}"><c:forEach begin="1" end="${data.RE_LEV}">&nbsp;&nbsp; </c:forEach>└</c:if>
									${data.TTL_NM}</a>
								</td>
			...
			<div class="component-pageing">
				<comTag:pagingIpro totalCount="${comFn:nvl(qnaListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
			</div> <!--// list_bottom E -->			
		</div>
	</form>
</div>
...
<form id="detailFrm" class="search_form" method="POST" >
	...
	<input type="hidden" name="resourceName" value="${comFn:nvl(param.resourceName, resourceName)}" >
	<input type="hidden" name="P_BBS_SECD"  id="P_BBS_SECD" value="" >
	<input type="hidden" name="P_BBS_SN"  id="P_BBS_SN" value="" >
	<input type="hidden" name="contextPath" value="${contextPath}" >
	<input type="hidden" name="P_USER_ID" value="${loginResult.USER_ID}" >
	<input type="hidden" name="P_PAGE_NO" value="${comFn:nvl(param.P_PAGE_NO, 1)}">
	<input type="hidden" name="P_PAGE_SIZE" value="${comFn:nvl(param.P_PAGE_SIZE, 10)}">
</form>