...
<script type="text/javascript" src="${jsPath}/opro/views/estm/estmCmtmProgList.js"></script> 
...
<div class="page-list">
	<form id="searchFrm" name="searchFrm" method="post">
		...
		<div class="form-setting-box">
			<div class="ui-setting">
				...
				<div class="form-setting">
					...
					<input type="text" id="P_ESTM_NM_S" name="P_ESTM_NM_S" value="${param.P_ESTM_NM_S }" class="component-input">
				</div>
				<!-- //Form Setting -->
			...
			<div class="ui-setting">
				...
				<div class="form-setting">
					...
					<!-- Select -->
					<comTag:comCmcdCdValueComboBox id="P_ESTM_PSCD_S" name="P_ESTM_PSCD_S" selectKey="${param.P_ESTM_PSCD_S}" cdId="ESTM_PSCD"  headerKey="" cond3="CMTM_PROG" headerValue="전체" className="component-select"/>
					<!-- //Select -->
				</div>
				<!-- Form Setting -->
			...
			<div class="table-option">
				...
				<div class="option-right">
					...
					<div class="table-num type-fleft">
						총 <strong>${comFn:nvl(estmCmtmProgListTotCnt, 0)}</strong>건
					</div>
					<a href="javascript:excelDwd('searchFrm', '/opro/estm/estmCmtmProgListExcelDwld.do','${comFn:nvl(estmCmtmProgListTotCnt, 0)}');" class="btn-download-s type-fleft">
			...
			<div style="overflow-x: scroll; overflow-y:hidden" >
				<table class="component-table">
					...
					<tbody>
						...
						<c:if test="${comFn:nvl(estmCmtmProgListTotCnt, 0) == 0}">
							<tr>
								<td colspan="9" class="txt-center">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
							</tr>
						</c:if>
						<c:if test="${estmCmtmProgListTotCnt > 0}">
							<c:forEach var="data" items="${estmCmtmProgList}" varStatus="status">
		...
		<div class="component-pageing">
			<comTag:pagingIpro totalCount="${comFn:nvl(estmCmtmProgListTotCnt,0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div>
		<!--//pageing -->
