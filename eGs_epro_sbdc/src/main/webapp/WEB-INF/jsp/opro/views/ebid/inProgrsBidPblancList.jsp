...
<script type="text/javascript" src="${jsPath}/opro/views/ebid/inProgrsBidPblancList.js"></script>
...
<div class="contents_wrap">
	...
	<form id="searchFrm" class="search_form" method="POST">
		...
		<input type="hidden" id="fngprtSuccAt" name="fngprtSuccAt" value="${param.P_FNGPRT_SUCC_AT }">
					...
					<table>
						...
						<tbody>
							...
							<tr>
								...
								<th>진행상태</th>
								<td>
									<comTag:comCmcdCdValueComboBox name="P_BID_PSCD_S" cdId="BID_PSCD" cond1="PART" selectKey="${param.P_BID_PSCD_S}" headerValue="전체" width="180" />
								</td>
							</tr>
							<tr>
	...
	<div class="list_wrap mt30" id="contentWrap">
		<div class="list_top">
			<p class="total">총 <span>${comFn:nvl(inProgrsBidPblancListTotcnt, 0)}</span>건</p>
		</div> <!--// list_top E -->
		<div class="list_conts">
			<table>
				<caption>발주계획</caption>
				<colgroup>
	                <col width="7%"/>
	                <col width="6%"/>
	                <col width="6%"/>
				...
				<tbody>
					<c:if test="${comFn:nvl(inProgrsBidPblancListTotcnt, 0) == 0}">
						<tr class="row">
							<td colspan="8">검색 조건에 해당하는 데이터가 존재하지 않습니다.</td>
						</tr>
					</c:if>
					<c:if test="${inProgrsBidPblancListTotcnt > 0}">
						<c:forEach var="data" items="${inProgrsBidPblancList}" varStatus="status">
							<tr class="row">
								...
								<td class="pl20 list_tit" onclick="inProgrsBidPblancDetail('${data.ANNC_NO}', '${data.ANNC_NGR}','${data.ROUND_NO }');" title="${data.BID_NM}" style="cursor: pointer;">
		...
		<div class="list_bottom">
			<comTag:pagingIpro totalCount="${comFn:nvl(inProgrsBidPblancListTotcnt, 0)}" pageNo="${comFn:nvl(param.P_PAGE_NO, 1)}" pageSize="${comFn:nvl(param.P_PAGE_SIZE, 10)}" clickPage="pageObj.clickPage"/>
		</div> <!--// list_bottom E -->
	</div> <!--// list_wrap E -->
</div>	
