...
<script type="text/javascript" src="${jsPath}/opro/views/comm/popup/bidWrtancDetail.js"></script> 
...
<div class="pop_wrap">
	...
	<div class="pop_container">
		<div class="view_wrap typeC">
				...
				<table>
					...
					<tbody>
				    	<tr>
				    		...
				    		<td>
				    			<c:if test="${empty inProgrsBidPblancDetail.BFAN_NO_LIST}">사전공고번호가 없습니다.</c:if>
				    			<c:if test="${not empty inProgrsBidPblancDetail.BFAN_NO_LIST}">${inProgrsBidPblancDetail.BFAN_NO_LIST }</c:if>
				    		</td>
			...
			<div class="view_area">
				<table>
			    	...
			    	<tr>
			            ...
			            <td>
			            	${inProgrsBidPblancDetail.CONT_SECD_NM}<c:if test="${inProgrsBidPblancDetail.UPRC_YN eq 'Y'}">(단가)</c:if>
			        ...
			        <tr>    
			            ...
			            <td>
			            	${inProgrsBidPblancDetail.G2B_NTFY_YN_NM}
			            </td>			            
					</tr>
			...
			<div class="view_area">
				<table>
			        ...
			        <tr>
						...
						<td>
		                    ...
		                    <div class="negoDiv" <c:if test="${inProgrsBidPblancDetail.SBID_MTCD ne '40'}"> style="display:none;"</c:if>>
		                    	<%-- 낙찰자선정방식이 [협상에의한계약]이 아닐 경우 활성화 --%>
	                        	기술점수비율&nbsp;${inProgrsBidPblancDetail.TCHN_SCR_RT }&nbsp;:&nbsp;가격점수비율&nbsp;${inProgrsBidPblancDetail.PRCE_SCR_RT }
								<%-- 낙찰자선정방식이 [협상에의한계약]이 아닐 경우 활성화 End--%>
		                    </div>
	                    </td>
			        </tr>
			        ...
			        <tr>    
			            ...
			            <%-- 입찰설명회 [예] 일 경우 활성화 --%>
			            <th scope="row"><label class="brfsDiv" <c:if test="${inProgrsBidPblancDetail.BID_BRFS_YN ne 'Y'}">style="display:none;"</c:if>>입찰설명회필수참석여부</label></th>
			            <td>
					...
					<tr class="ctrcDiv siteBrfsDiv" <c:if test="${inProgrsBidPblancDetail.CONT_SECD ne '2' and inProgrsBidPblancDetail.SITE_BRFS_YN ne 'Y'}">style="display:none;"</c:if>>    
					...
					<c:if test="${(inProgrsBidPblancDetail.BID_MTCD eq 'BID') or (inProgrsBidPblancDetail.BID_MTCD eq 'OFF')}">
			...
			<%-- 계약방법이 [지명경쟁]일 경우 활성화 --%>
	        <div class="slctDiv" <c:if test="${inProgrsBidPblancDetail.CONT_MTCD ne '10001' and inProgrsBidPblancDetail.CONT_MTCD ne '10005'}"> style="display: none;"</c:if>>
	        	<div class="tit_area">
	        ...
	        <%-- 계약방법이 [제한경쟁] 일 경우 활성화 --%>
	        <div class="lmtDiv" <c:if test="${inProgrsBidPblancDetail.CONT_MTCD ne '10002'}"> style="display: none;"</c:if>>
				<div class="tit_area">
			...
			<div class="view_area">
	            <table>
	                ...
	                <tr>
	                    ...
	                    <%-- 계약구분이 [구매] 일 경우 활성화 --%>
	                    <th scope="row"><label class="buyDiv" <c:if test="${inProgrsBidPblancDetail.CONT_SECD ne '0'}"> style="display: none;"</c:if>>물품인도조건</label></th>
	                    <td>
