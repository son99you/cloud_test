...
<script type="text/javascript" src="${jsPath}/opro/views/noti/gnrlNoticeList.js"></script>
...
<div class="contents_wrap">
	...
	<div class="tit_wrap">
		<h3 class="tit">입찰공지사항</h3>
	</div>
	  
	<form id="searchFrm" class="inbiFrm" method="POST" action="${contextPath}/elbi/myBidPblancList.do">
		...
		<fieldset>
			...
			<div class="list_wrap mt30">
				...
				<div class="list_conts">
					<table class="tableList" summary="공지사항 목록입니다">
							...
							<c:if test="${noticeBoardListTotCnt > 0}">
								<c:forEach var="data" items="${noticeBoardList}" varStatus="status" >
									<tr class="row" onclick="detailInqire('${data.NTT_SN}');" style="cursor: pointer;">
										<td>${data.RNUM}</td>
										<td class="txtl pl20"><c:if test="${data.MURE_CD eq 'M' }"><font color="red">[필독]</font></c:if>${data.SJ_NM}</td>
