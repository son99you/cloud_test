...
public class ComCmcdCtprvnComboBoxTag extends RequestContextAwareTag {

//	private static final long serialVersionUID = -9202778299779483697L;

	@SuppressWarnings("unused")
	private static final Logger LOG = LoggerFactory.getLogger(ComCmcdCtprvnComboBoxTag.class);
	...
	private String objectId 		= "";												//combobox 이벤트 발생 시 호출될 객체 ID
	...
	protected int doStartTagInternal() throws Exception {
		...
		sb.append(createFunctionScript());
	...
	private List<FwkDataEntity> cdValueListInqire(){
		...
		trans = comCmcdLegaldongService.legaldongBrtcListInqire(parameterMap);		
	...
	private String createFunctionScript() throws IOException {

		StringBuffer sb = new StringBuffer();		

		if( FwkStringUtil.isNull(this.objectId) == false &&  !"".equals(this.objectId)){

			String context = super.getRequestContext().getContextPath();
...
//			sb.append(".dd .ddArrow{width:16px;height:16px; margin-top:-8px; background:url(" + context + "/statics/images/select_down_arrow.gif) no-repeat;}						\r\n");
...
//			sb.append("<script type=\"text/javascript\" src=\"" + context + "/statics/libs/jquery.select.min.js\"></script>									\r\n");
			sb.append("<script type=\"text/javascript\"> 																														\r\n");
			sb.append("	(function() {																																					\r\n");
			sb.append("		" + this.id + "_legaldongPageObj = {};																												\r\n");
			sb.append("		" + this.id + "_legaldongPageObj.setEventHandler = function() {																			\r\n");

//			sb.append("			$(\"#" + this.id + "\").msDropdown({roundedCorner: \"false\"});																		\r\n");

			sb.append("			$(\"#" + this.id + "\").on(\"change\", function() {																							\r\n");

//			sb.append("			$(\"#" + this.objectIdㄴ + "\").msDropdown({roundedCorner: \"false\"}).data(\"dd\").destroy();								\r\n");
			sb.append("				var inHtml = \"\";																																	\r\n");

			if(FwkStringUtil.isNull(this.headerValue) == false){	
				sb.append("			inHtml += '<option value=\"" + this.headerKey + "\">" + this.headerValue + "</option>' 									\r\n");
			}

			sb.append("				if( $(this).val() == '' || $(this).val() == '" + this.headerKey + "' ){																	\r\n");
			sb.append("					$(\"#" + this.objectId + "\").html(inHtml); 																							\r\n");
			sb.append("					$(\"#" + this.objectId + "\").change();			 																						\r\n");
			sb.append("				}else{																																				\r\n");
			sb.append("		 			var actionUrl = \"/com/cmcd/legaldongSignguListInqire\";																			\r\n");
			sb.append("					FwkCmmnUtil.submitAjax(actionUrl, {P_AREA_NM : $(this).val() }, function(res) {										\r\n");

			sb.append("						var list_size = res.areaList.length; 																									\r\n");
			sb.append("						for(var i = 0; i < list_size; i++){ 																										\r\n");
			sb.append("							inHtml += '<option value=' + res.areaList[i].AREA_NM + '>' + res.areaList[i].AREA_NM + '</option>'; 		\r\n");
			sb.append("						}																																				\r\n");

			sb.append("						$(\"#" + this.objectId + "\").html(inHtml); 																						\r\n");
			sb.append("					},																																					\r\n");
			sb.append("					function(){},																																	\r\n");
			sb.append("					function(){	//ajax call back 후 실행 함수																								\r\n");
			sb.append("					});																																				\r\n");
			sb.append("				}																																						\r\n");
//			sb.append("				$(\"#" + this.objectId + "\").msDropDown().data(\"dd\");																				\r\n");
			sb.append("			});																																						\r\n");
			sb.append("		}																																								\r\n");

			sb.append("		$(function(){																																				\r\n");
			sb.append("			" + this.id + "_legaldongPageObj.setEventHandler();																						\r\n");
			sb.append("		});																																							\r\n");
			sb.append("})();																																								\r\n");
			sb.append("</script>																																						\r\n");
		}
