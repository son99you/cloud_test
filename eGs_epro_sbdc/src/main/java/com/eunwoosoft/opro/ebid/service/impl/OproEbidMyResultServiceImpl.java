 ...
 * @version : 1.0
 */
@Service("oproEbidMyResultService")
public class OproEbidMyResultServiceImpl extends AbstractFwkService implements OproEbidMyResultService {
	 ...
	 */
	@Override
	public FwkTransferObject myBidResultListWithPgng(final FwkParameterMap parameterMap) {
		...
		trans.put(OproEbidMyResultService.MY_BID_RESULT_LIST, 
				oproEbidMyResultDao.selectMyBidResultListWithPgng(parameterMap));
		trans.put(OproEbidMyResultService.MY_BID_RESULT_LIST_TOTCNT, 
				oproEbidMyResultDao.selectMyBidResultListTotcnt(parameterMap));
	...
	@Override
	public FwkTransferObject myBidResultDetailInqire(final FwkParameterMap parameterMap) {
		...
		//협상에의한 계약 (기술평가, 기술가격종합평가) 일때
		
		if(!"70".equals(biMst.getString("SBID_MTCD"))){
			trans.put(OproEbidResultService.SCSBID_PREARNGER_LIST
					, oproEbidMyResultDao.selectNtatScsbidPrearngerList(parameterMap));
		}else{
			trans.put(OproEbidResultService.SCSBID_PREARNGER_LIST
					, oproEbidMyResultDao.selectDatpVendList(parameterMap));
		}
